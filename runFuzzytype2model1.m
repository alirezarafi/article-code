...
segmentLen      = 2.5;            % [m] مقدار دلخواه (2–3 m خوب است)
...
totalL  = cumDist(end);
    ...
    % --------- برای ذخیره داده جهت تیون فازی ----------  (ADD)
trainIn      = [];   % ورودی‌های کنترلر مسیر: [turnDeg , goalDist]
trainOut     = [];   % خروجی ولتاژهای متناظر   : [V_left  , V_right]
trainInAvoid = [];   % ← فقط اگر می‌خواهید fisAvoid را هم تیون کنید
trainOutAvoid= [];   % --------------------------------------------- (END)


    %% Noise parameters  (σ = انحراف معیار)
noise.lidar     = 0.05;     % [m]  — نویز محدودهٔ هر پرتو
noise.heading   = deg2rad(1);  % [rad] — خطای ژیروسکوپ/کدِر
noise.process_v = 0.01;     % نویز ولتاژ موتور (فرآیند)
rng(1);                     % برای تکرارپذیری
...
distanceToCheckpoint_array = [];
    ...
    state = state_initial;
        ...
        % === پارامترهای توقف ربات بر اثر موانع متحرک ===
    isRobotStopped  = false;   % وضعیت فعلی ربات
    stopDist        = 1.7;     % اگر فاصله تا مانع متحرک < این مقدار → توقف
    resumeDist      = 1.5;     % اگر همه موانع متحرک ≥ این فاصله باشند → ادامه
...
avoidThreshold = 2.0;    % [m] اگر فاصله جلویی کمتر از این شود، سوئیچ به کنترل‌گر اجتناب

numRules   = numel(fis.Rules);       % تعداد کل قواعد
ruleCount  = zeros(numRules,1);      % دفعات شلیک
ruleEnergy = zeros(numRules,1);      % جمع درجهٔ شلیک
...
avoidEnterDist = 2.4;      % ورود به حالت اجتناب
avoidExitDist  =2.8;      % خروج از حالت اجتناب
...
% فاصلهٔ فعلی تا هدف نهایی
goalDist = norm(currentLocation - finalGoal);
% ذخیره در آرایه برای رسم/تحلیل بعدی
if timeStep == 1
...
% --- بدون سنسور عقب: مقادیر جایگزین بزرگ (یعنی «پشت خالیه») ---
d_bl = 10;   % تقریباً = حداکثر برد
d_b  = 10;
d_br = 10;
...
elseif  isRobotStopped && minDistMovObs >= resumeDist
    ...
    angleError = wrapToPi(angleToGoal - currentHeadingAngle);
    % چون تابع فازی شما از -400 تا 400 تنظیم شده، به درجه تبدیل می‌کنیم
    turning_angle_deg = rad2deg(angleError);
 % ثبت مقادیر خطا برای شاخص‌سازی
    angle_error_array(end+1) = abs(angleError);  % ثبت خطای زاویه (به رادیان)
   

% محاسبه‌ی زاویه برای هر دو حالت
turning_angle_deg = rad2deg(angleError);
goalDist = norm(currentLocation - finalGoal);
...
ruleEnergyAvoid = ruleEnergyAvoid + rfA;
    ...
    controlMode = 1;
    % ورودی‌های a108: [turning_angle, goalDist]
    inNormal = [turning_angle_deg, goalDist];
[outNormal,~,~,~,rfN] = evalfis(fis, inNormal);      % ← rfN درجهٔ شلیک قواعد مسیر
voltage_left  = max(-15, min(outNormal(1),  15));
voltage_right = max(-15, min(outNormal(2), 15));
% ----- LOG for FIS-tuning (ADD) -----
trainIn(end+1,:)  = inNormal;     % [turnDeg , goalDist]
trainOut(end+1,:) = outNormal;    % [V_L , V_R] قبل از اشباع
% ------------------------------------ (END)

% --- ثبت آمار قواعد مسیر ---
rfN = max(rfN,[],2);                  % در صورت نوع‑۲
ruleCount   = ruleCount   + (rfN > 0);
ruleEnergy  = ruleEnergy  + rfN;
    ...
    % مدل دینامیکی ربات (در اینجا همان قبلی که شما داشتید)
...
plot(path_x, path_y, 'k--','LineWidth',1.5);
    ...
    %% === گزارش و حذف قواعد کم‌استفاده ===
minUse    = 1;                                        % آستانهٔ حداقل دفعات
energyThr = 0.01;                                     % 1٪ انرژی کل
unused    = find(ruleCount == 0);
rare      = find( ruleEnergy/sum(ruleEnergy) < energyThr & ruleCount > 0);
rules2Del = unique([unused; rare]);
% --------- SAVE training data (ADD) ----------
trainingData = [trainIn , trainOut];       % مسیر
save('trainingData_DF.mat','trainingData');

if ~isempty(trainInAvoid)                  % ← اگر لاگ کرده‌اید
    trainingDataAvoid = [trainInAvoid , trainOutAvoid];
    save('trainingDataAvoid_DF.mat','trainingDataAvoid');
end
...
fprintf('Unused rules   : %s\n', mat2str(unused'));
fprintf('Low-energy (<1%%) rules: %s\n', ...
        mat2str(setdiff(rare,unused)'));

if ~isempty(rules2Del)
    fisPruned = fis;
    fisPruned.Rules(rules2Del) = [];      % حذف
    save('fis_pruned.mat','fisPruned');   % ذخیره برای اجراهای بعدی
    fprintf('Rules removed  : %d  →  Remaining: %d\n', ...
            numel(rules2Del), numel(fisPruned.Rules));
else
    fprintf('No rule qualified for deletion.\n');
end
...
%% محاسبه شاخص‌های خطا (برای زاویه و فاصله)
timeError = (0:stepSize_time:(length(angle_error_array)-1)*stepSize_time);  % بردار زمان مرتبط با شاخص‌های خطا
...
% انتگرال از قدر مطلق خطا (Integral Absolute Error, IAE)
IAE_angle = trapz(timeError, abs(angle_error_array));


% انتگرال از مربع خطا (Integral Squared Error, ISE)
...
% ===== Speed error metrics (analogous to angle error) =====
% 1) انتخاب سرعت مرجع:
v_ref_const = 1.0;                 % [m/s] مقدار ثابت مرجع (به دلخواه شما)
v_meas = speed_mps;                % سرعت اندازه‌گیری‌شدهٔ اسکالر
if exist('speed_mps_smooth','var')
    v_meas = speed_mps_smooth;     % اگر صاف‌شده را ترجیح می‌دهید
end

% هم‌ترازسازی طول‌ها
Nv = min(numel(v_meas), numel(t_vec));
v_meas = v_meas(1:Nv);
t_v    = t_vec(1:Nv);

% پروفایل مرجع ساده (ثابت)
v_ref = v_ref_const*ones(1, Nv);

% (اختیاری) پروفایل مرجع تطبیقی: کاهش سرعت نزدیک هدف
% فقط اگر 'goalDist_array' را قبلاً پر کرده‌اید:
% if exist('goalDist_array','var') && numel(goalDist_array)>=Nv
%     v_max = v_ref_const;                 % سقف سرعت
%     d     = goalDist_array(1:Nv);        % فاصله تا هدف
%     d0    = 2.0;                          % آستانه کاهش سرعت (m)
%     scale = min(1, d./d0);               % بین 0 و 1
%     v_ref = max(0.2, v_max.*scale);      % مرجع با کاهش نرم، حداقل 0.2 m/s
% end

% 2) خطای سرعت
speed_error_array = v_ref(:) - v_meas(:);

% 3) محاسبه شاخص‌ها
% (مثل زاویه) محور زمانِ مرتبط با سیگنال سرعت
timeSpeed = t_v;                            
...
fprintf('\n--- Speed Error Metrics ---\n');
fprintf('MSE (speed):  %.4f (m/s)^2\n', MSE_speed);
fprintf('IAE (speed):  %.4f (m/s)*s\n', IAE_speed);
fprintf('ISE (speed):  %.4f (m/s)^2*s\n', ISE_speed);
fprintf('MAE (speed):  %.4f m/s\n', MAE_speed);
fprintf('RMSE(speed):  %.4f m/s\n', RMSE_speed);

% (اختیاری) رسم مرجع و اندازه‌گیری و خطا
figure('Name','Speed Reference vs Measured');
plot(t_v, v_ref, 'LineWidth', 1.8); hold on; grid on;
plot(t_v, v_meas, 'LineWidth', 1.8);
legend('v_{ref}','v_{meas}','Location','best');
xlabel('Time (s)'); ylabel('Speed (m/s)');
title('Speed Reference vs Measured');

figure('Name','Speed Error e_v(t)');
plot(t_v, speed_error_array, 'LineWidth', 1.8); grid on;
xlabel('Time (s)'); ylabel('e_v(t) = v_{ref}-v_{meas} (m/s)');
title('Speed Error');
...
label = 'FUZZYm2';  % <-- برای هر کنترلر تغییر بده
...
fprintf('Saved fuzzym2-run pack -> run_%s.mat\n', label);
